...
impl Into<*mut mbedtls_sys::x509_crt> for &mut Certificate {
    fn into(self) -> *mut mbedtls_sys::x509_crt {
        &mut *self.inner
    }
}

impl Into<*const mbedtls_sys::x509_crt> for &mut Certificate {
    fn into(self) -> *const mbedtls_sys::x509_crt {
        &*self.inner
    }
}

impl Into<*mut mbedtls_sys::x509_crt> for &Certificate {
    fn into(self) -> *mut mbedtls_sys::x509_crt {
        &*self.inner as *const _ as *mut _
    }
}

impl Into<*const mbedtls_sys::x509_crt> for &Certificate {
    fn into(self) -> *const mbedtls_sys::x509_crt {
        &*self.inner
    }
}
...
impl Certificate {
    ...
    pub fn push_back_der(&mut self, der: &[u8]) -> Result<()> {
        unsafe { x509_crt_parse_der(self.into(), der.as_ptr(), der.len()) }.into_result()?;
        Ok(())
    }
    ...
    pub fn push_back(&mut self, pem_or_der: &[u8]) -> Result<()> {
        unsafe { x509_crt_parse(self.into(), pem_or_der.as_ptr(), pem_or_der.len()) }.into_result()?;
        Ok(())
    }
