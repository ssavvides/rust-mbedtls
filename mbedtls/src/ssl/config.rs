...
pub struct Config {
    ...
    #[cfg(not (feature = "threading"))]
    verify_callback: Option<Arc<dyn (Fn(LinkedCertificate, i32, &mut VerifyError) -> Result<()>) + 'static>>,
    ...
    #[cfg(not (feature = "threading"))]
    ca_callback: Option<Arc<dyn (Fn(&LinkedCertificate, &mut ForeignOwnedCertListBuilder) -> Result<()>) + 'static>>,
...
impl Into<*const mbedtls_sys::ssl_config> for &Config {
    fn into(self) -> *const mbedtls_sys::ssl_config {
        &*self.inner
    }
}
...
impl Config {
    ...
    pub fn set_dh_params(&mut self, params: &[u8]) -> Result<()> {
        ...
        unsafe {
            ssl_conf_dh_param_ctx(self.into(), (&*ctx).into())
                .into_result()
                .map(|_| ())
        }
    }
        ...
        F: Fn(LinkedCertificate, i32, &mut VerifyError) -> Result<()> + Send + Sync + 'static,
    {
        ...
        unsafe { ssl_conf_verify(self.into(), Some(verify_callback::<F>), Arc::as_ptr(&mut self.verify_callback.as_ref().unwrap()) as *mut F as _) }
    }

    #[cfg(not(feature = "threading"))]
    pub fn set_verify_callback<F>(&mut self, cb: F)
    where
        F: Fn(LinkedCertificate, i32, &mut VerifyError) -> Result<()> + 'static,
    {
        self.verify_callback = Some(Arc::new(cb));
        unsafe { ssl_conf_verify(self.into(), Some(verify_callback::<F>), Arc::as_ptr(&mut self.verify_callback.as_ref().unwrap()) as *mut F as _) }
    }
        ...
        F: Fn(&LinkedCertificate, &mut ForeignOwnedCertListBuilder) -> Result<()> + Send + Sync + 'static,
    {
        ...
        unsafe { ssl_conf_ca_cb( self.into(), Some(ca_callback::<F>), Arc::as_ptr(&mut self.ca_callback.as_ref().unwrap()) as *mut F as _) }
    }

    #[cfg(not(feature = "threading"))]
    pub fn set_ca_callback<F>(&mut self, cb: F)
    where
        F: Fn(&LinkedCertificate, &mut ForeignOwnedCertListBuilder) -> Result<()> + 'static,
    {
        self.ca_callback = Some(Arc::new(cb));
        unsafe { ssl_conf_ca_cb( self.into(), Some(ca_callback::<F>), Arc::as_ptr(&mut self.ca_callback.as_ref().unwrap()) as *mut F as _) }
    }
        ...
        F: (Fn(i32, &str, i32, &str) -> ()) + Send + Sync + 'static,
    {
        ...
        unsafe { ssl_conf_dbg(self.into(), Some(dbg_callback::<F>), Arc::as_ptr(&mut self.dbg_callback.as_ref().unwrap()) as *mut F as _) }
    }
